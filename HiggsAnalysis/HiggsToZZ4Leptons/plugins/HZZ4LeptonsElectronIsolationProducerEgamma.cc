...
void HZZ4LeptonsElectronIsolationProducerEgamma::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    auto_ptr<reco::GsfElectronCollection> isolatedElectrons(new reco::GsfElectronCollection);
    auto_ptr<reco::GsfElectronRefVector> isolatedElectronRefs(new reco::GsfElectronRefVector);
    
    Handle<reco::GsfElectronCollection> electronsH;
    iEvent.getByLabel(electronTag, electronsH);
    reco::GsfElectronCollection electrons = *electronsH;
    ...
    for(std::size_t i = 0; i < electronsH->size(); i++) {
        ...
        reco::GsfElectronRef electronRef(electronsH,i);
