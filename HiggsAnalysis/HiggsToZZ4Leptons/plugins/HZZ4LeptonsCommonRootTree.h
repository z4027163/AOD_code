...
class HZZ4LeptonsCommonRootTree : public edm::EDAnalyzer {
 ...
 public:
  ...
  void ReadParameters(const edm::ParameterSet& pset){ 
    ...
    // LumiSummary
    LumiSummarySrc_           = consumes<LumiSummary,edm::InLumi>(pset.getParameter<edm::InputTag>("LumiSummarySrc"));
    ...
    pfmetT1Tag_             = consumes<vector<reco::PFMET> >(pset.getParameter<edm::InputTag>("PfMETT1Label"));
  ...
  void DefineBranches(TTree *Tree_){
    ...
    Tree_->Branch("RECOELE_ASSPACKPFCAND",RECOELE_ASSPACKPFCAND,"RECOELE_ASSPACKPFCAND[100]/F");
    ...
    Tree_->Branch("RECOELE_parScl_E",RECOELE_parScl_E,"RECOELE_parScl_E[100]/F"); 
    ...
    Tree_->Branch( "RECO_PFJET_PUID", RECO_PFJET_PUID, "RECO_PFJET_PUID[100]/I");
    Tree_->Branch( "RECO_PFJET_PUID_MVA", RECO_PFJET_PUID_MVA, "RECO_PFJET_PUID_MVA[100]/F");
    //Tree_->Branch( "RECO_PFJET_QGTagger_LH",RECO_PFJET_QGTagger_LH,"RECO_PFJET_QGTagger_LH[200]/F");
...
/*     Tree_->Branch( "RECO_CALOMETOPT",       &calometopt,       "RECO_CALOMETOPT/F"); */
    ...
    //Particle Flow MET with T1 correction 
    Tree_->Branch( "RECO_PFMETT1", &pfmett1, "RECO_PFMETT1/F");
    Tree_->Branch( "RECO_PFMETT1_X", &pfmett1_x, "RECO_PFMETT1_X/F");
    Tree_->Branch( "RECO_PFMETT1_Y", &pfmett1_y, "RECO_PFMETT1_Y/F");
    Tree_->Branch( "RECO_PFMETT1_PHI", &pfmett1_phi, "RECO_PFMETT1_PHI/F");
    Tree_->Branch( "RECO_PFMETT1_THETA", &pfmett1_theta, "RECO_PFMETT1_THETA/F");
  ...
  void Initialize(){
    ...
    RECO_PFJET_N = 0;
    for (int ijets=0;ijets<100;ijets++) {
      ...
      //RECO_PFJET_QGTagger_LH[ijets]=-999.;
    }
...
/*     iEvent.getByLabel  (EleID_HZZTightTag_ , eleIdHZZCutHandles[3]) ; */
    ...
    for (edm::View<reco::GsfElectron>::const_iterator cand = EleRefs->begin(); 
	 cand != EleRefs->end(); ++cand) {
      ...
      //RECOELE_ASSPACKPFCAND[index] = cand->p4().associatedPackedPFCandidates();
      ...
      //reco::SuperClusterRef parSclRef = cand->parentSuperCluster();
      ...
      //RECOELE_parScl_E[index]= parSclRef->energy();
	...
	//<< "  E_parScl=" << RECOELE_parScl_E[index]
	<< std::endl;
  ...
  void fillMuons(const edm::Event& iEvent,const edm::EventSetup& iSetup){
    ...
    for (edm::View<reco::Muon>::const_iterator cand = MuCandidates->begin(); cand != MuCandidates->end(); ++cand) {
      ...
      //      RECOMU_isME0Muon[indexbis]=cand->isME0Muon();
      ...
      std::cout << "\n Muon in the event: "
		...
		<< "  isTHPt=" << RECOMU_isTrackerHighPtMu[indexbis]
		<< "  isME0=" << RECOMU_isME0Muon[indexbis]
		<< std::endl;
...
//      RECOMU_PFchHad[indexbis]  = (cand->pfIsolationR04().sumChargedHadronPt);
//      RECOMU_PFneuHad[indexbis] = (cand->pfIsolationR04().sumNeutralHadronEt);
//      RECOMU_PFphoton[indexbis] = (cand->pfIsolationR04().sumPhotonEt);
//      RECOMU_PFPUchAllPart[indexbis]= (cand->pfIsolationR04().sumPUPt);
...
/* 	      << "  caloMETho="        << calometho */
    ...
    iEvent.getByToken(pfmetT1Tag_,pfmetT1Handle);
    for ( reco::PFMETCollection::const_iterator i=pfmetT1Handle->begin(); i!=pfmetT1Handle->end(); i++) {
      pfmett1       = i->pt();     
      pfmett1_x     = i->px();
      pfmett1_y     = i->py();
      pfmett1_phi   = i->phi();
      pfmett1_theta = i->theta();
    }
  ...
  void filljets(const edm::Event& iEvent){
    ...
    //edm::Handle<edm::ValueMap<float> > QGTaggerMC_LH;
    //edm::Handle<edm::ValueMap<float> > QGTaggerData_LH;
    ...
    if (fillMCTruth == 1){
      ...
      //iEvent.getByToken(QGTaggerMC_LHTag_,QGTaggerMC_LH);
    }
    else{
      iEvent.getByToken(jetsDataTag_, pfjets);
      iEvent.getByToken(jetsMVATag_, pfjetsmva);
      iEvent.getByToken(PuJetMvaDatafullDiscr_,puJetIdMVAData);
      //iEvent.getByToken(QGTaggerData_LHTag_, QGTaggerData_LH);      
	...
	if(fabs(i->eta())>3.){
      ...
      /* float qgLH=0.; */
      /* if (fillMCTruth == 1) { */
      /*   qgLH=(*QGTaggerMC_LH)[pfjetref]; */
      /* } */
      /* else { */
      /*   qgLH=(*QGTaggerData_LH)[pfjetref]; */
      /* } */
      ...
      //RECO_PFJET_QGTagger_LH[index_jets] = qgLH;
	...
	//<< "QGTagger_LH="<< RECO_PFJET_QGTagger_LH[index_jets]
	<< endl;
  ...
  void fillBTagging(const edm::Event& iEvent){
	...
	cSV_BTagJet_ET[l]=btagIter->first->et();
 ...
 private:
  ...
  // LumiSummary
  edm::EDGetTokenT<LumiSummary> LumiSummarySrc_;
  ...
  edm::EDGetTokenT<vector<reco::PFJet> > jetsTag_, jetsDataTag_, jetsMVATag_;
  ...
  // QG discriminator
  //edm::EDGetTokenT<edm::ValueMap<float> > QGTaggerMC_LHTag_,QGTaggerData_LHTag_;
  ...
  edm::EDGetTokenT<vector<reco::PFMET> >  pfmetT1Tag_;
  ...
  float RECOELE_E[100],RECOELE_PT[100],RECOELE_PTError[100],RECOELE_P[100],RECOELE_ETA[100],RECOELE_THETA[100],RECOELE_PHI[100],RECOELE_MASS[100],RECOELE_ASSPACKPFCAND[100];
  ...
  float
    RECOELE_scl_E[100],RECOELE_scl_Et[100],RECOELE_scl_Eta[100],RECOELE_scl_Phi[100], RECOELE_parScl_E[100];
  ...
  // RECO JETS
  int RECO_PFJET_N, RECO_PFJET_CHARGE[100],RECO_PFJET_PUID[100];
  float RECO_PFJET_ET[100], RECO_PFJET_PT[100], RECO_PFJET_ETA[100], RECO_PFJET_PHI[100],RECO_PFJET_PUID_MVA[100];
    //RECO_PFJET_QGTagger_LH[200];
  ...
  float pfmett1,pfmett1_x,pfmett1_y,pfmett1_phi,pfmett1_theta;
